...
from attr import ib
custlist=[{'name': '전인호', 'gender': 'M', 'email': 'seintjin@naver.com', 'birth': '1999'},
          {'name': '김나리', 'gender': 'F', 'email': 'cksdncjoasd@naver.com', 'birth': '1999'},
          {'name': '홍길동', 'gender': 'F', 'email': 'qwnjd12@naver.com', 'birth': '1999'}  ]
page= len(custlist) - 1
...
while True:
    ...
    if choice=="I":  
        ...
        if page <= 0 :
            print('첫번째 페이지 입니다. 이전 페이지 이동 불가 합니다.')
        else:
            page -= 1
            print(f'현재 페이지는 {page+1}페이지 입니다.')
        print(page) 
        ...
        if page >= len(custlist) -1 :
            print('마지막 페이지 입니다. 다음 페이지로 이동 불가합니다.')
        else:
            page += 1
